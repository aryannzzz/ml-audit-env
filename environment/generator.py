...
Builds all 36 experiments at import time:
- 18 violated (6 easy, 6 medium, 6 hard)
- 12 adversarially clean
...
VALID_ARCHETYPES = ["tabular_clf", "timeseries_reg", "tabular_multi", "tabular_survival"]
...
def inject_V1(exp: dict) -> dict:
    ...
    """
    # Keep metadata neutral; the violation should be inferred from code order.
...
def inject_V2_feature_engineering(exp: dict) -> dict:
    ...
    exp["feature_engineering"]["code_snippet"] = (
        "# Compute lag features on the full sequence\n"
        "df['demand_lag_7'] = df['energy_demand_kwh'].shift(7)\n"
        "df['demand_ma_7']  = df['energy_demand_kwh'].rolling(7).mean()\n"
        "# Then split AFTER computing features\n"
...
def generate(
    ...
    exp["experiment_id"] = f"{archetype}_seed{seed}_{violation_str}"
    exp["_seed"] = seed
...
def _build_pool() -> dict[str, list[dict]]:
    ...
    for (va, vb), seed in zip(COMPOUND_PAIRS, COMPOUND_SEEDS):
        ...
        base_id = f"compound_{va}_{vb}_{seed}"
        base["experiment_id"] = base_id
        base["id"] = base_id
        base["_seed"] = seed
        base["_is_clean"] = False

        # Ensure compound episodes expose the same inspectable artifacts as
        # standard generated episodes, otherwise agents cannot inspect at all.
        base_artifacts = [
            "dataset_info",
            "preprocessing",
            "split_config",
    ...
    # Additional clean experiments for diversity
...
# Build pool at import time — critical for cold-start performance
POOL = _build_pool()
